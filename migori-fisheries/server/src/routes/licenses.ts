...
const parseLicenseType = (value: unknown): LicenseType | undefined => {
  ...
  return map[normalized] ?? (Object.values(LicenseType).includes(normalized.toUpperCase() as LicenseType) ? (normalized.toUpperCase() as LicenseType) : undefined);
};

const parseLicenseStatus = (value: unknown, expiryDate: Date): LicenseStatus => {
  const normalized = parseText(value)?.toLowerCase().replace(/[^a-z]/g, "");
  const map: Record<string, LicenseStatus> = {
...
type ParsedLicenseImportRow = {
  rowNumber: number;
  licenseNo: string;
  receiptNo: string;
  createData: Prisma.LicenseCreateManyInput;
  updateData: Prisma.LicenseUncheckedUpdateInput;
};
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const parsedRows: ParsedLicenseImportRow[] = [];
    ...
    for (const [index, rawRow] of rows.entries()) {
      ...
      const licenseNo =
        parseText(pickValue(row, ["uniquenumber", "licenseno", "licensenumber", "uniqueid"])) ??
      ...
      if (!licenseNo || !receiptNo || !holderName || !holderIdNumber || !holderPhoneNumber || !subCountyRaw || !wardRaw || !type || !issuedDate || !expiryDate) {
        rowErrors.push(
          formatImportError(
            rowNumber,
            "required fields missing (Unique Number or Receipt No, Name, ID, Phone, Sub-County, Ward, Type, Issued Date, Expiry Date)."
          )
        );
        continue;
      }
      ...
      const createData: Prisma.LicenseCreateManyInput = {
        licenseNo,
        receiptNo,
        holderName,
        holderIdNumber,
    ...
    const existingByReceiptNo = new Map(
      existingLicenses
        .filter((license) => license.receiptNo)
        .map((license) => [license.receiptNo as string, license])
    );
    const createRows: ParsedLicenseImportRow[] = [];
    const updateRows: Array<ParsedLicenseImportRow & { existingId: string }> = [];
