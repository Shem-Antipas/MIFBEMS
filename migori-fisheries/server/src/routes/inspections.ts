...
const allowedExtensionPhotoTypes = ["image/png", "image/jpeg", "image/webp"] as const;
...
router.get(
  "/",
  authorize(["DIRECTOR", "ADMIN", "FISHERIES_OFFICER", "FARMER"]),
      ...
      req.user.role === "FISHERIES_OFFICER"
        ...
        : req.user.role === "FARMER"
          ? {
              ...
              farmName: { contains: farmerUser?.name ?? "__no_farmer_name__", mode: "insensitive" as const }
            }
          : {};
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const payload = req.body as z.infer<typeof createInspectionSchema>;
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    const payload = req.body as z.infer<typeof updateInspectionSchema>;
...
router.patch(
  ...
  auditLog("INSPECTION", "APPROVAL"),
  asyncHandler(async (req, res) => {
    const { id } = req.params as z.infer<typeof idParamSchema>;
    const payload = req.body as z.infer<typeof updateApprovalSchema>;
    const inspection = await prisma.inspection.findUnique({ where: { id } });
    ...
    const updated = await prisma.inspection.update({
      ...
      data: {
        ...
        approvedAt: new Date(),
        result:
          payload.approvalStatus === ExtensionApprovalStatus.APPROVED
