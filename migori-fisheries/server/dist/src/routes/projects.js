...
const isProjectSubCounty = (value) => value === COUNTY_WIDE || MIGORI_SUBCOUNTIES.includes(value);
const normalizeWard = (subCounty, ward) => {
...
const createProjectSchema = projectPayloadSchema.strict().superRefine((value, ctx) => {
    ...
    if (value.subCounty !== COUNTY_WIDE &&
        ward !== "Unspecified" &&
        ward !== ALL_WARDS &&
...
router.post("/import", authorize(["DIRECTOR", "ADMIN"]), auditLog("PROJECT", "BULK_IMPORT"), asyncHandler(async (req, res) => {
    ...
    rows.forEach((rawRow, index) => {
        ...
        if (!name || !canonicalSubCounty || budget === undefined || !startDate) {
            rowErrors.push(`Row ${rowNumber}: required fields missing (Project Name/Title, Sub-County, Approved Project Cost, Project Start Date).`);
            return;
        }
        ...
        if (canonicalSubCounty !== COUNTY_WIDE &&
            ward !== "Unspecified" &&
            ward !== ALL_WARDS &&
...
router.put("/:id", validate({ params: idParamSchema, body: updateProjectSchema }), authorize(["DIRECTOR", "ADMIN"]), auditLog("PROJECT"), asyncHandler(async (req, res) => {
    ...
    if (targetSubCounty !== COUNTY_WIDE &&
...
router.delete("/:id", validate({ params: idParamSchema }), authorize(["DIRECTOR", "ADMIN"]), auditLog("PROJECT"), asyncHandler(async (req, res) => {
    const { id } = req.params;
